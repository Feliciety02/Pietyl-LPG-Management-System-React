...
import { Package, Tag, Layers, Building2 } from "lucide-react";
...
function safeMoney(v) {
  if (v == null) return "";
  return decimalOnly(String(v).trim());
}
...
export default function EditProductModal({
  ...
  useEffect(() => {
    ...
    setUnitPrice(safeMoney(product?.supplier_cost));
  ...
  const sku = useMemo(() => {
    ...
    return "";
  }, [type, sizeClean, stoveType, name, existingSkus, product?.sku]);
  ...
  const canSubmit = useMemo(() => {
    ...
    return true;
  }, [product, name, sku, priceClean, unitPriceClean, supplierId, type, sizeClean, stoveType, negativePrice, negativeUnitPrice]);
